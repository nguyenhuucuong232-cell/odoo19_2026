...
class SaleOrder(models.Model):
    ...
    # -- Fields for Report Header/Footer --
    quote_content = fields.Text(string="Nội dung báo giá chính", default="Kính gửi Quý Khách hàng bảng báo giá dịch vụ...")
    
    # -- Fields for Logic --
    have_frequency = fields.Boolean(string="Có cột Tần suất?", default=False)
    
    # -- Contact & Executor --
    contact_id = fields.Many2one('res.partner', string="Người liên hệ (Khách)")
    # user_id is already in sale.order (Salesperson)

    # -- Discount Logic --
    customer_discount = fields.Monetary(string="Chiết khấu khách hàng", currency_field='currency_id')
    price_after_discount = fields.Monetary(string="Sau chiết khấu", compute='_compute_price_after_discount')

    # -- QR Code (Placeholder for now, can be computed) --
    ...
    def _cron_update_sequence(self):
        """Cron method to update sale order sequences if needed."""
        # Search for all sale orders and re-compute their names/sequences if needed
        # For now, just pass to allow module installation
        pass
